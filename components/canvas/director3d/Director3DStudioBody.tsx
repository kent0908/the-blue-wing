...
const JOINT_GROUPS: { label: string; joints: JointName[] }[] = [
  { label: k("軀幹"), joints: ["hips", "spine", "chest", "neck", "head"] },
  { label: k("左臂"), joints: ["leftShoulder", "leftUpperArm", "leftForearm", "leftHand"] },
  { label: k("右臂"), joints: ["rightShoulder", "rightUpperArm", "rightForearm", "rightHand"] },
  { label: k("左腿"), joints: ["leftHip", "leftThigh", "leftShin", "leftFoot"] },
  { label: k("右腿"), joints: ["rightHip", "rightThigh", "rightShin", "rightFoot"] },
];

const BODY_STYLES: BodyStyle[] = ["mannequin", "stick"];

const TAB_LABEL: Record<EditorTab, string> = { attribute: k("屬性"), posture: k("姿勢"), path: k("路徑"), camera: k("鏡頭") };
  ...
  return (
    <div className="flex min-h-0 flex-1">
      ...
      <div className="flex w-[200px] shrink-0 flex-col gap-3 overflow-y-auto border-r border-[#1c1c1c] p-3">
        <div>
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] text-[#8a8a8a]">{tr("角色")}</span>
          ...
          <div className="space-y-1">
            {scene.characters.map((ch) => (
              ...
              >
                <span className="flex-1 truncate">{tr(ch.name)}</span>
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className="border-t border-[#1e1e1e] pt-3">
          <div className="mb-1.5 flex items-center justify-between text-[11px] text-[#8a8a8a]">
            <span>{tr("快速運鏡")}</span>
            {shot && <span className="text-[#7ff0cd]">{tr("移動中…")}</span>}
          </div>
          ...
          <p className="mt-1.5 text-[10px] leading-relaxed text-[#6d6d6d]">{tr("選好角色再點按鈕，鏡頭會自動移過去；之後仍可拖曳、滾輪微調。")}</p>
        </div>
        ...
        <div className="border-t border-[#1e1e1e] pt-3">
          ...
          {!recording && !recordedClip && (
            <>
              <Slider label={tr("錄製時長（秒）")} value={recordSeconds} min={1} max={30} onChange={setRecordSeconds} />
              ...
              <p className="mt-1.5 text-[10px] leading-relaxed text-[#6d6d6d]">
                {cameraTrack.keyframes.length || cameraTrack.follow
                  ? tr("已設定鏡頭軌道：錄製時鏡頭會自動照「鏡頭」分頁的軌道跑，角色也會照路徑走。")
                  : tr("錄製期間可以自由拖曳、滾輪、點運鏡按鈕，畫面怎麼動就錄成怎樣——時間到會自動停止。想要精準的運鏡，到右邊「鏡頭」分頁設定軌道。")}
              </p>
            </>
          )}
          ...
          {!recording && recordedClip && (
            <div className="mt-1.5 space-y-1.5">
              ...
              {onExportFramesForVideo && (
                ...
                >
                  {exportingFrames ? tr("傳送中…") : tr("送去影片生成（運鏡影片＋畫面＋文字）")}
                </button>
              )}
              <p className="text-[10px] leading-relaxed text-[#6d6d6d]">
                {tr("這段錄製會直接當「運鏡影片參考」送給 Seedance 2.0 / 2.5（僅這兩個版本支援，其他模型會自動忽略、退回只用畫面）； 同時也會把錄製時抽出的")} {recordedFrames.length} {tr("張畫面當多重參考圖，加上自動判讀的運鏡文字提示")}
                {cameraMoveHint ? tr("（目前判讀：「{h}」，可在輸入框自行修改）", { h: cameraMoveHint }) : ""}
                {tr("一起帶到輸入框。生成服務對參考影片有解析度下限，畫面太小的視窗錄出來可能會被拒絕，建議放大瀏覽器視窗再錄。")}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="border-t border-[#1e1e1e] pt-3">
          <div className="mb-1.5 text-[11px] text-[#8a8a8a]">{tr("地面")}</div>
          ...
          <div className="mt-1.5">
            <Slider label={tr("透明度")} value={Math.round(scene.ground.opacity * 100)} min={0} max={100} onChange={(v) => setScene((s) => ({ ...s, ground: { ...s.ground, opacity: v / 100 } }))} />
          </div>
          <div className="mt-1.5">
            <Slider label={tr("高度")} value={Math.round(scene.ground.height * 100)} min={-100} max={100} onChange={(v) => setScene((s) => ({ ...s, ground: { ...s.ground, height: v / 100 } }))} />
          </div>
        </div>

        <div className="border-t border-[#1e1e1e] pt-3">
          <div className="mb-1.5 text-[11px] text-[#8a8a8a]">{tr("背景顏色")}</div>
          <input
            type="color"
      ...
      <div className="relative min-w-0 flex-1 bg-[#0a0a0a]">
        <Suspense fallback={<div className="grid h-full place-items-center text-[13px] text-[#6d6d6d]">{tr("載入 3D 場景中…")}</div>}>
          <Director3DScene
            scene={scene}
        ...
        {placing && (
          <div className="pointer-events-none absolute left-1/2 top-3 -translate-x-1/2 rounded-full bg-[#f0c27f] px-3 py-1 text-[11px] font-medium text-[#1a1408] shadow-lg">
            {tr("點擊放置模式：在地面點一下新增路徑點 · 拖曳可旋轉視角 · Esc 結束")}
          </div>
        )}
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2 rounded-full border border-[#2a2a2a] bg-[#111]/90 px-2 py-1.5 backdrop-blur">
          <button
            ...
            aria-label={playing ? tr("停止") : tr("播放時間軸")}
          >
            {playing ? "■" : "▶"}
          </button>
          <input
        ...
        {recording && (
          <div className="absolute left-3 top-3 flex items-center gap-1.5 rounded-full bg-black/70 px-2.5 py-1 text-[11px] text-[#ff9b9b]">
            ...
            {tr("錄製中")} {recordElapsed}/{recordSeconds}s
          </div>
        )}
        {captured && (
          <div className="absolute bottom-14 right-3 w-[160px] overflow-hidden rounded-lg border border-[#3a3a3a] shadow-2xl">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={captured} alt={tr("截圖預覽")} className="w-full" />
            <div className="bg-black/70 px-2 py-1 text-center text-[10px] text-[#c9c9c9]">{tr("已截圖，儲存後會用這張")}</div>
          </div>
        )}
      </div>
      ...
      <div className="flex w-[260px] shrink-0 flex-col border-l border-[#1c1c1c]">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {!selected && tab !== "camera" && <p className="text-[12px] text-[#6d6d6d]">{tr("選一個角色來編輯")}</p>}

          {selected && tab === "attribute" && (
            <div className="space-y-3">
              <label className="block">
                <div className="mb-1 text-[10.5px] text-[#8a8a8a]">{tr("名字")}</div>
                <input value={tr(selected.name)} maxLength={20} onChange={(e) => updateSelected({ name: e.target.value })} className={fieldCls} />
              </label>

              <div>
                <div className="mb-1 text-[10.5px] text-[#8a8a8a]">{tr("體型")}</div>
              ...
              <label className="block">
                <div className="mb-1 text-[10.5px] text-[#8a8a8a]">{tr("Color（僅預覽辨識，不影響最終圖像）")}</div>
          ...
          {selected && tab === "posture" && (
            <div className="space-y-4">
              <div>
                <div className="mb-1.5 text-[10.5px] text-[#8a8a8a]">{tr("POSE 預設姿勢庫")}</div>
                <div className="grid grid-cols-2 gap-1.5">
                  {POSE_NAMES.map((name) => (
                    <button
                      key={name}
              ...
              <div className="border-t border-[#1e1e1e] pt-3">
                <div className="mb-1.5 text-[10.5px] text-[#8a8a8a]">{tr("調整參數（RIG）— 精細姿勢控制")}</div>
                <div className="space-y-3">
                  {JOINT_GROUPS.map((group) => (
                    <div key={tr(group.label)}>
                      <div className="mb-1 text-[10.5px] text-[#7ff0cd]">{tr(group.label)}</div>
          ...
          {selected && tab === "path" && (
            <div className="space-y-3">
              ...
              >
                {placing ? tr("■ 結束點擊放置（Esc）") : tr("🖱 點擊放置路徑點")}
              </button>
              <p className="text-[10px] leading-relaxed text-[#6d6d6d]">
                {placing
                  ? tr("在地面每點一下就多一個路徑點，時間會自動接在上一點之後。第一點會把角色搬過去當起點。")
                  : tr("開啟後，在畫面的地面點一下就放一個點——比拖曳黃球精準得多。放好的點可以點選（清單同步反白）、拖曳微調或在下面改時間／姿勢。")}
              </p>

              <div className="grid grid-cols-2 gap-1.5">
                <label className="block">
                  <div className="mb-1 text-[10px] text-[#8a8a8a]">{tr("每點間隔（秒）")}</div>
                  <input
                    type="number"
                    step={0.5}
                    min={0.5}
              ...
              <div className="space-y-2">
                {(selected.path ?? []).length === 0 && (
                  <p className="rounded-lg bg-[#161616] px-2 py-3 text-center text-[11px] text-[#6d6d6d]">
                    {tr("還沒有路徑點——按上面「點擊放置」，然後在地面依序點出路線")}
                  </p>
                )}
              ...
              >
                {playing ? tr("■ 停止") : tr("▶ 預覽路徑（{s}s）", { s: pathDuration(selected.path) })}
              </button>
            </div>
          )}
          ...
          {tab === "camera" && (
            <div className="space-y-3">
              <p className="text-[10px] leading-relaxed text-[#6d6d6d]">
                {tr("鏡頭軌道＝幾個「第幾秒鏡頭在哪、看哪裡」的關鍵影格，中間自動平滑運鏡（環繞會走弧線、推拉沿視線）。 最快的做法：先用左邊「快速運鏡」或拖曳擺好起始視角，再點一個運鏡模板，時長跟著左邊「錄製時長」（目前")} {recordSeconds}s）。
              </p>

              <div>
                <div className="mb-1.5 flex items-center justify-between text-[10.5px] text-[#8a8a8a]">
                  <span>{tr("運鏡模板")}</span>
                  <span className="text-[#6d6d6d]">{tr("以")} {(selected ?? scene.characters[0])?.name ?? tr("原點")} {tr("為主體")}</span>
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                  {CAMERA_MOVE_TEMPLATES.map((tpl) => {
                    ...
                    return (
                      ...
                      >
                        <div>{tr(tpl.label)}</div>
                        <div className="text-[9.5px] text-[#7d7d7d]">{tr(tpl.hint)}</div>
                      </button>
                    );
                  })}
                </div>
                {cameraTrack.follow && (
                  <p className="mt-1.5 rounded-lg bg-[#161616] px-2 py-1.5 text-[10px] leading-relaxed text-[#8ab4ff]">
                    {cameraTrack.follow.mode === "chase" ? tr("跟拍") : tr("定點跟蹤")}{tr("中：")}
                    {scene.characters.find((c) => c.id === cameraTrack.follow!.characterId)?.name ?? tr("（角色已刪除）")}
                    {tr("。再點一次同一個按鈕可取消；角色需要有路徑才看得出效果。")}
                  </p>
                )}
              </div>
              ...
              <div className="border-t border-[#1e1e1e] pt-3">
                <div className="mb-1.5 flex items-center justify-between text-[10.5px] text-[#8a8a8a]">
                  <span>{tr("關鍵影格（")}{cameraTrack.keyframes.length}）</span>
                ...
                <button
                  ...
                  className="w-full rounded-lg bg-gradient-to-r from-[#7ff0cd] to-[#4fd1c5] px-2 py-1.5 text-[11px] font-medium text-[#0a1a16] hover:brightness-105 disabled:opacity-40"
                >
                  {tr("＋ 把目前視角記成第")} {fmtSec(playhead)}{tr("s 的影格")}
                </button>
                <p className="mt-1.5 text-[10px] leading-relaxed text-[#6d6d6d]">
                  {tr("手動做法：拖時間軸到某一秒 → 拖曳／滾輪擺好鏡頭 → 按上面記下來，重複幾次就是一段運鏡。畫面上的藍色小相機是每個影格的位置，點它可以跳過去看。")}
                </p>
                    ...
                    <div key={i} className="rounded-lg bg-[#161616] p-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <select
                          ...
                          aria-label={tr("緩動")}
                        >
                          <option value="smooth">{tr("緩入緩出")}</option>
                          <option value="linear">{tr("等速")}</option>
                        </select>
                        <button type="button" onClick={() => removeCameraKeyframe(i)} className="text-[#6d6d6d] hover:text-[#ff8a8a]" aria-label={tr("刪除影格")}>
                          <IconTrash className="h-3.5 w-3.5" />
                        </button>
                      </div>
                      <div className="mt-1.5 grid grid-cols-2 gap-1.5">
              ...
              >
                {playing ? tr("■ 停止") : tr("▶ 預覽運鏡（{s}s）", { s: fmtSec(timelineDuration) })}
              </button>
            </div>
          )}
        </div>
