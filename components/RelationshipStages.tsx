...
export default function RelationshipStages({ affection, kind = "romance" }: { affection: number; kind?: LadderKind }) {
  ...
  const meter = kind === "trust" ? "信賴度" : "好感度";
  ...
  return <section aria-label="關係階段" className="space-y-4 p-4">
    <div className="rounded-xl border border-[#25473f] bg-[#11251f] p-3">
      <p className="text-xs text-[#93b9b0]">{kind === "trust" ? "目前信賴" : "目前關係"} · {meter} {actual}</p>
      <h3 className="mt-1 text-base font-medium text-[#9af2da]">{current.name}</h3>
      <p className="mt-2 text-xs leading-5 text-[#a9bbb7]">{current.nextMin === null ? "已達最高階段，繼續累積彼此的故事。" : `距離下一階段還需要 ${Math.max(0, current.nextMin - actual)} ${meter}。`}</p>
    </div>
    ...
    <div className="flex flex-col gap-2" role="group" aria-label="預覽各關係階段">
      {AFFECTION_LEVELS.map((stage, index) => {
        ...
        return <button type="button" key={stage.min} aria-pressed={selectedIndex === index}
          ...
          <span className="min-w-0"><span className="block text-xs font-medium text-[#e0ebe8]">{stage.name}</span><span className="mt-1 block text-[10px] text-[#9caba7]">{stage.min} {meter} · {index === current.index ? "目前" : reached ? "已達成" : "未解鎖"}</span></span>
        </button>;
      })}
    </div>

    <div className="rounded-xl border border-[#293238] bg-[#11181b] p-3" aria-live="polite">
      <p className="text-[11px] text-[#8da39b]">{selectedIndex === current.index ? "目前階段說明" : "階段預覽"}{locked ? ` · 尚需 ${selected.min - actual} ${meter}` : ""}</p>
      <h4 className="mt-1 text-sm font-medium">{selected.name}</h4>
      <p className="mt-2 text-xs leading-6 text-[#acbbb6]">{selected.unlock}。</p>
      <p className="mt-2 text-[11px] leading-5 text-[#798f86]">點選僅預覽說明，不會改變{meter}或解鎖階段。</p>
