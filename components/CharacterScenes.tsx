...
export default function CharacterScenes({ characterId, onClose, refreshKey }: { characterId: number; onClose: () => void; refreshKey?: number }) {
  ...
  const preview = async (kind: "image" | "video") => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result?.error?.message || tr("無法取得點數預覽"));
      setQuote(result.quote);
    } catch (e) { setError(e instanceof Error ? e.message : tr("無法取得點數預覽")); } finally { setQuoting(false); }
  };

  const load = useCallback(() =>
    fetch(`/api/characters/${characterId}/scenes`)
      .then(async (r) => { const j = await r.json(); if (!r.ok) throw new Error(j?.error?.message || tr("載入失敗")); return j; })
      .then(setData)
      .catch((e) => setError(e instanceof Error ? e.message : tr("載入失敗"))), [characterId]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      Promise.all(data.pending!.map(async (pending) => {
        ...
        const result = await res.json();
        if (!res.ok || result.status === "failed") throw new Error(result?.error?.message || tr("場景生成失敗"));
      })).then(() => { if (alive) void load(); }).catch(e => { if (alive) setError(e instanceof Error ? e.message : tr("查詢場景失敗")); });
    }, 4000);
  ...
  const generate = async (kind: "image" | "video") => {
    ...
    setBusyLabel(kind === "image" ? tr("生成圖片場景中…") : tr("生成影片場景中…"));
    try {
      ...
      if (!response.ok) { if (response.status === 409) setQuote(null); throw new Error(result?.error?.message || tr("生成失敗")); }
      setQuote(null);
      for (let i = 0; result.status === "processing" && i < 200; i++) {
        ...
        if (!poll.ok || next.status === "failed") throw new Error(next?.error?.message || tr("生成失敗"));
        result = { ...next, requestId: result.requestId };
      }
      if (result.status !== "completed") throw new Error(tr("生成仍在處理，請稍後重新查看場景"));
      await load();
    } catch (e) { setError(e instanceof Error ? e.message : tr("生成失敗")); }
  ...
  return (
    <aside className="flex min-h-0 w-full min-w-0 flex-col border-l border-[#1c1c1c] bg-black">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-6">
        {error && <p role="alert" className="my-3 text-xs text-[#ff9b9b]">{tr(error)}</p>}
        {!!data?.pending?.length && <p className="my-3 text-xs text-[#7ff0cd]">{tr("場景生成中，正在更新進度…")}</p>}
        {data?.reason && <p className="my-3 text-xs text-[#a0aaa6]">{tr(data.reason)}</p>}
        ...
        {data && !data.unlocked && (
          <div className="rounded-xl border border-[#3a2e18] bg-[#1a150c] p-4 text-center">
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-[#f0c27f]">
              {tr("角色專屬圖片／影片場景是高階方案的功能，升級後就能為每個關係階段生成獨一無二的畫面。")}
            </p>
            <Link
              href="/account"
        ...
        {data && data.unlocked && data.eligible && (
          <>
            ...
            {quote && <section aria-label={tr("場景生成確認")} className="mt-4 space-y-3 rounded-xl border border-[#7ff0cd55] bg-[#10201a] p-3 text-xs">
              ...
              <div className="flex flex-wrap gap-2"><button type="button" disabled={!!busy} onClick={() => generate(quote.kind)} className="flex-1 rounded-full bg-[#7ff0cd] px-3 py-2 font-medium text-[#0b1712] disabled:opacity-50">{tr("確認扣")} {quote.credits} {tr("點並生成")}</button><button type="button" disabled={!!busy} onClick={() => setQuote(null)} className="rounded-full border border-white/20 px-3 py-2">{tr("取消")}</button></div>
            </section>}
