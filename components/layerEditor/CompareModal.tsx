  ...
  return (
    <div className="fixed inset-0 z-[210] flex flex-col bg-black/95">
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-[#1c1c1c] px-4">
        <span className="text-[13px] font-medium text-white">{tr("前後比對")}</span>
        ...
        <button type="button" onClick={onClose} aria-label={tr("關閉")} className="grid h-8 w-8 place-items-center rounded-full text-[#8a8a8a] hover:bg-[#1f1f1f] hover:text-white">
        ...
        {mode === "side" ? (
          <>
            <figure className="flex min-h-0 max-w-[48%] flex-col items-center gap-1">
              ...
              <img src={before} alt={tr("修改前")} className="max-h-[80vh] max-w-full rounded-lg object-contain" style={checker} />
              <figcaption className="text-[11px] text-[#8a8a8a]">{tr("修改前")}</figcaption>
            </figure>
            <figure className="flex min-h-0 max-w-[48%] flex-col items-center gap-1">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={after} alt={tr("修改後")} className="max-h-[80vh] max-w-full rounded-lg object-contain" style={checker} />
              <figcaption className="text-[11px] text-[#7ff0cd]">{tr("修改後")}</figcaption>
            </figure>
          </>
        ) : (
          <div className="relative max-h-[82vh] max-w-full select-none overflow-hidden rounded-lg" style={checker}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={after} alt={tr("修改後")} className="block max-h-[82vh] max-w-full" draggable={false} />
            <div className="absolute inset-0 overflow-hidden" style={{ width: `${split}%` }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={before} alt={tr("修改前")} className="block max-h-[82vh] max-w-none" draggable={false} style={{ width: `${10000 / split}%` }} />
            </div>
            <div className="pointer-events-none absolute inset-y-0 w-0.5 bg-[#7ff0cd]" style={{ left: `${split}%` }} />
            <span className="pointer-events-none absolute left-2 top-2 rounded bg-black/70 px-1.5 py-0.5 text-[10px] text-white">{tr("修改前")}</span>
            <span className="pointer-events-none absolute right-2 top-2 rounded bg-black/70 px-1.5 py-0.5 text-[10px] text-[#7ff0cd]">{tr("修改後")}</span>
            <input type="range" min={1} max={100} value={split} onChange={(e) => setSplit(Number(e.target.value))} className="absolute inset-x-0 bottom-3 mx-auto w-[60%] accent-[#7ff0cd]" aria-label={tr("比對分割位置")} />
          </div>
        )}
      </div>
    </div>
