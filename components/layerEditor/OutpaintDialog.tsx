...
const PRESETS: { label: string; margins: OutpaintMargins }[] = [
  { label: k("四周 25%"), margins: { top: 0.25, right: 0.25, bottom: 0.25, left: 0.25 } },
  { label: k("四周 50%"), margins: { top: 0.5, right: 0.5, bottom: 0.5, left: 0.5 } },
  { label: k("左右 50%（變橫）"), margins: { top: 0, right: 0.5, bottom: 0, left: 0.5 } },
  { label: k("上下 50%（變直）"), margins: { top: 0.5, right: 0, bottom: 0.5, left: 0 } },
  { label: k("只往下"), margins: { top: 0, right: 0, bottom: 0.6, left: 0 } },
  { label: k("只往上"), margins: { top: 0.6, right: 0, bottom: 0, left: 0 } },
];
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/85 p-4">
      <div className="flex w-full max-w-[760px] flex-col overflow-hidden rounded-2xl border border-[#2a2a2a] bg-[#111]">
        <div className="flex h-12 shrink-0 items-center gap-2 border-b border-[#1c1c1c] px-4">
          <span className="text-[13px] font-medium text-white">{tr("擴圖")}</span>
          <span className="text-[11px] text-[#6d6d6d]">{tr("把畫面往外延伸，AI 補上原本邊界外的內容")}</span>
        ...
        <div className="flex gap-5 p-4">
          <div className="flex w-[340px] shrink-0 flex-col items-center justify-center">
            <div className="relative rounded-lg border border-dashed border-[#7ff0cd66] bg-[#0a0a0a]" style={{ width: pw, height: ph }}>
              ...
              <span className="pointer-events-none absolute -bottom-5 left-0 text-[10px] text-[#6d6d6d]">{tr("虛線框＝擴圖後的範圍")}</span>
            </div>
          </div>
          <div className="min-w-0 flex-1 space-y-3">
            <div>
              <div className="mb-1 text-[10.5px] text-[#8a8a8a]">{tr("快速選擇")}</div>
              <div className="flex flex-wrap gap-1.5">
                {PRESETS.map((p) => (
                  <button key={tr(p.label)} type="button" onClick={() => setMargins(p.margins)} className="rounded-md bg-[#1f1f1f] px-2 py-1 text-[11px] text-[#c9c9c9] hover:bg-[#282828]">
                    {tr(p.label)}
                  </button>
                ))}
              </div>
            ...
            <label className="block">
              <div className="mb-1 text-[10.5px] text-[#8a8a8a]">{tr("延伸出去的內容（可改）")}</div>
            ...
            >
              {submitting ? tr("擴圖中…") : tr("開始擴圖")}
            </button>
            <p className="text-[10px] leading-relaxed text-[#6d6d6d]">
              {tr("固定使用 Seedream 5.0 pro（和局部重繪同一個編輯端點，計一次點數）。結果會取代這個圖層並自動放大成延伸後的大小，原本的畫面保留在中間；可以用「比對」檢查、「回復」還原。")}
            </p>
          </div>
        </div>
      </div>
