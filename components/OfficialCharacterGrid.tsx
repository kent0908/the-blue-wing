...
export default function OfficialCharacterGrid() {
  ...
  useEffect(() => {
    ...
    fetch("/api/official-characters", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error(tr("載入官方角色失敗")))))
  ...
  const open = async (c: OfficialCharacter) => {
    ...
    try {
      ...
      if (!r.ok) throw new Error(j?.error?.message || tr("無法開始聊天"));
      router.push(`/companions/${j.character.id}`);
    } catch (e) {
      setError(e instanceof Error ? e.message : tr("無法開始聊天"));
      setOpening(null);
    }
  };
  ...
  return (
    <div>
      <p className="mb-4 text-[13px] leading-relaxed text-[#8a8a8a]">
        {tr("私立蒼翼高中 二年A班——整班被拉進荒廢的太古修真世界，只有一群高中生和他們從現代帶來的知識與特長。 點一位同學開始聊天，好感度、記憶和對話都是你自己的進度。")}
      </p>
      ...
      {!signedIn && (
        <p className="mb-3 rounded-lg border border-[#2a2a2a] bg-[#141414] px-3 py-2 text-[12.5px] text-[#c9c9c9]">
          {tr("可以先瀏覽角色，")}<Link href="/login?next=%2Fcompanions" className="text-[#7ff0cd] underline">{tr("登入")}</Link>{tr("後才能開始聊天。")}
        </p>
      )}
          ...
          >
            <div className="relative h-[68%] w-full overflow-hidden bg-[#f4f4f4]">
              {c.idleVideoUrl && c.idleVideoStatus === "completed" && signedIn ? (
                ...
                <img src={c.avatarSrc} alt={tr(c.name)} className="h-full w-full object-cover object-top" />
              )}
              <span className="absolute left-2 top-2 rounded-full bg-[#1e2a3d]/90 px-2 py-0.5 text-[10px] text-[#8ab4ff]">{tr("官方")}</span>
              <span className="absolute right-2 top-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] text-white" title={c.contentRating === "all_ages" ? tr("全年齡角色：只有友誼與夥伴互動") : tr("成年角色")}>
                {c.age} {tr("歲")}{c.contentRating === "all_ages" ? tr(" · 全年齡") : ""}
              </span>
            </div>
