...
export default function OfficialTemplates(){
 ...
 return <section id="official-templates" aria-label="官方電影模板" className="mt-10">
  ...
  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map(item=><article key={item.id} className="overflow-hidden rounded-2xl border border-[#29342d] bg-[#101713]">
   <div className="relative"><TemplatePreview id={item.id} revision={item.revision} title={item.title} active={playing===item.id} onToggle={()=>setPlaying(playing===item.id?null:item.id)}/><span className="pointer-events-none absolute left-3 top-3 rounded-full bg-black/65 px-2 py-1 text-[10px] text-white">{item.category}</span></div>
