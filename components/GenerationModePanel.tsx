...
function ReviewedAssets({ selected, onSelected }: { selected: number[]; onSelected: (ids:number[])=>void }) {
  ...
  return <div className="max-h-[55dvh] overflow-y-auto p-3 text-xs" aria-label={tr("已審核素材清單")}>
    ...
    {loading && <p>{tr("載入中…")}</p>}{error && <p role="alert" className="text-red-300">{tr(error)}</p>}
    {!loading && !error && !assets.length && <p className="text-[#999]">{tr("尚無已登錄素材。")}</p>}
    <div className="grid gap-2">{!loading && assets.map(asset=><label key={asset.id} className="flex min-w-0 items-center gap-2 rounded border border-white/10 p-2">
      ...
      <span className="min-w-0 flex-1 truncate">{tr(asset.name)}</span><span className="shrink-0 text-[10px] text-[#999]">{asset.status!=="active"?tr("未可用"):asset.assetType!=="image"?tr("尚未開放"):tr("可使用")}</span>
    </label>)}</div>
    <div className="mt-3 flex flex-wrap items-center gap-3"><button type="button" disabled={loading||page<=1} onClick={()=>navigate(page-1)}>{tr("上一頁")}</button><span>{page} / {pages}</span><button type="button" disabled={loading||page>=pages} onClick={()=>navigate(page+1)}>{tr("下一頁")}</button><button type="button" onClick={()=>onSelected([])}>{tr("清除選取")}</button></div>
  </div>;
}
