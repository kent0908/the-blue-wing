...
const STEPS = [k("角色靈感"), k("外觀細節"), k("個性與關係"), k("故事與開場"), k("確認角色")];
const HOBBIES = [k("電影"), k("音樂"), k("閱讀"), k("電玩"), k("攝影"), k("旅行"), k("烹飪"), k("繪畫"), k("運動"), k("園藝"), k("貓"), k("咖啡")];
...
export default function CharacterBuilder({ character, onClose, onSaved }: {
  ...
  const [profile, setProfile] = useState<CharacterProfile>(() => character?.profile ? readProfile(character.profile) : { ...EMPTY_PROFILE, style: tr("動漫"), species: tr("人類"), speaking: tr("自然簡短"), relationship: tr("新朋友") });
  ...
  useEffect(() => {
    ...
    const nav = stepNav.current;
    const revealStep = () => {
      const active = nav?.querySelector<HTMLElement>('[aria-current="step"]');
      if (nav && active) nav.scrollLeft += active.getBoundingClientRect().left - nav.getBoundingClientRect().left - (nav.clientWidth - active.offsetWidth) / 2;
    };
    revealStep();
  ...
  useEffect(() => {
    ...
    fetch("/api/assets", { signal: controller.signal })
      .then(async r => {
        if (!r.ok) throw new Error(r.status === 401 ? tr("請先登入後再建立角色") : tr("素材載入失敗，請關閉後重試"));
        return r.json();
      })
  ...
  const fields = (keys: ProfileKey[]) => keys.map(key => {
    ...
    return <div key={key} className="flex min-w-0 flex-col">
      <label htmlFor={`character-${key}`} className="text-sm text-white/70">{tr(spec.label)}</label>

      {spec.max >= 300 ? <textarea id={`character-${key}`} className={fieldClass} rows={3} maxLength={spec.max} value={profile[key]} onChange={e => set(key, e.target.value)} placeholder={`自訂${tr(spec.label)}（選填）`} />
        : <input id={`character-${key}`} className={fieldClass} maxLength={spec.max} value={profile[key]} onChange={e => set(key, e.target.value)} placeholder={`自訂${tr(spec.label)}（選填）`} />}
      {"options" in spec && <div className="mt-2 flex flex-wrap gap-2" role="group" aria-label={tr("{label}預設", { label: tr(spec.label) })}>
  ...
  function go(next: number) {
    if (next > step && (!name.trim() || !Number.isInteger(profile.age) || profile.age < 18 || profile.age > 120)) {
      setError(tr("請填寫角色名字，以及 18 至 120 歲的整數年齡")); setStep(0); return;
    }
  ...
  async function save() {
    ...
    setError("");
    try {
      if (!name.trim()) throw new Error(tr("請填寫角色名字"));
      ...
      if (!res.ok) throw new Error(result.error?.message ?? tr("儲存失敗，請稍後再試"));
      onSaved(result.character);
    } catch (e) { setError(e instanceof Error ? e.message : tr("儲存失敗")); }
  ...
  return <dialog ref={dialog} aria-labelledby="character-builder-title" onCancel={e => { e.preventDefault(); if (!saving) onClose(); }} className="fixed inset-0 m-auto max-h-[94dvh] w-[min(1100px,96vw)] max-w-none overflow-hidden rounded-3xl border border-white/15 bg-[#101313] p-0 text-white shadow-2xl backdrop:bg-black/80">
    <div className="flex max-h-[94dvh] flex-col">
      <header className="flex shrink-0 items-center justify-between border-b border-white/10 px-5 py-4 sm:px-8">
        <div><p className="text-[10px] tracking-[.25em] text-[#7ff0cd]">THE BLUE WING · CHARACTER</p><h2 id="character-builder-title" className="mt-1 text-lg">{character ? tr("編輯你的角色") : tr("讓角色，從想像走進對話")}</h2></div>
          ...
          <section className="min-w-0 space-y-6">
            ...
            <fieldset disabled={saving} className="min-w-0 space-y-6">
              {step === 0 && <>
                ...
                <label className="block text-sm text-white/70">{tr("成年角色年齡 *")}<input type="number" min={18} max={120} step={1} className={fieldClass} value={Number.isNaN(profile.age) ? "" : profile.age} onChange={e => setProfile(p => ({ ...p, age: e.target.valueAsNumber }))} /></label>
              ...
              {step === 2 && <>
                ...
                <p className="text-xs text-white/40">{tr("說話方式調整文字回覆語氣，不是語音播放。")}</p>
                <label className="block text-sm text-white/70">{tr("補充人設")}<textarea className={fieldClass} rows={3} maxLength={2000} value={personality} onChange={e => setPersonality(e.target.value)} placeholder={tr("習慣、價值觀、特殊經歷…")} /></label>
                <div><label htmlFor="character-likes" className="text-sm text-white/70">{tr("興趣與喜好")}</label><div className="mt-2 flex flex-wrap gap-2">{HOBBIES.map(hobby => {
              ...
              {step === 4 && <>
                ...
                <div className="flex gap-2" role="group" aria-label={tr("設定總覽分類")}>{(["appearance", "story"] as const).map(tab => <button type="button" key={tab} aria-pressed={summaryTab === tab} onClick={() => setSummaryTab(tab)} className={`rounded-full px-5 py-2 text-sm ${summaryTab === tab ? "bg-[#7ff0cd]/15 text-[#7ff0cd]" : "bg-white/5 text-white/50"}`}>{tab === "appearance" ? tr("外觀") : tr("個性與故事")}</button>)}</div>
                <dl className="grid gap-3 sm:grid-cols-2">{(summaryTab === "appearance" ? appearance : story).map(key => <div key={key} className="rounded-xl border border-white/10 p-4"><dt className="text-xs text-white/40">{PROFILE_FIELDS[key].label}</dt><dd className="mt-2 whitespace-pre-wrap break-words text-sm">{profile[key] || tr("未設定")}</dd></div>)}</dl>
                {summaryTab === "story" && <div className="space-y-3 text-sm text-white/60"><p className="whitespace-pre-wrap break-words">{tr("補充人設：")}{personality || tr("未設定")}</p><p className="break-words">{tr("喜好：")}{likes || tr("未設定")}</p></div>}
              </>}
            </fieldset>
          </section>
        </div>
      ...
      <footer className="shrink-0 border-t border-white/10 bg-[#101313] px-5 py-4 sm:px-8">
        {error && <p role="alert" className="mb-3 text-sm text-red-300">{tr(error)}</p>}
        <div className="flex items-center justify-between gap-3"><button type="button" disabled={saving || step === 0} onClick={() => go(step - 1)} className="rounded-full border border-white/15 px-5 py-3 text-sm disabled:opacity-30">{tr("上一步")}</button>
          <button type="button" disabled={saving} onClick={() => step === 4 ? void save() : go(step + 1)} className="rounded-full bg-[#7ff0cd] px-6 py-3 text-sm font-medium text-[#0d201b] disabled:opacity-50">{saving ? tr("儲存中…") : step === 4 ? character ? tr("儲存設定") : tr("建立並開始聊天") : tr("下一步 →")}</button></div>
      </footer>
    </div>
  </dialog>;
}
