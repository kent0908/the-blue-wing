...
export default function ModelFunctionMenu({ models, mode, selectedModel, selectedOperation, onSelect, loading, error }: {
  ...
  return <div className={styles.root} aria-label={tr("模型與功能選單")}>
    <div ref={modelPanel} className={styles.list} aria-label={tr("模型清單")}>
      <div className={styles.heading}>{t.nav.modelsHeading}</div>
      {error && <p role="alert" className={styles.empty}>{tr(error)}</p>}
      {!models.length && <p className={styles.empty}>{loading ? t.nav.loading : tr("此模式目前沒有可用模型")}</p>}
      {models.map(model => {
    ...
    <div ref={functionPanel} className={styles.functions} aria-label={tr("模型功能")} onKeyDown={event => { if (event.key === 'ArrowLeft') { event.preventDefault(); modelPanel.current?.querySelector<HTMLButtonElement>('button[aria-expanded=true]')?.focus(); } }}>
      <div className={styles.heading}>{visible ? modelLabel(visible.displayName || getImageModel(visible.id)?.name || visible.id) : t.nav.modelFunctions}</div>
      {functions.map(item => <button key={item.id} type="button" className={styles.function} disabled={!item.enabled} aria-pressed={selectedModel === visible?.id && selectedOperation === item.id} onClick={() => { if (visible && item.enabled) onSelect(visible.id, item.id); }}>
        {modeLabel(t, item.id, item.label)}{selectedModel === visible?.id && selectedOperation === item.id ? ' ✓' : ''}
        {!item.enabled && <small>{tr(item.reason) || tr("尚未開放")}</small>}
      </button>)}
      {!functions.length && <p className={styles.empty}>{tr("點選模型即可使用標準生成。")}</p>}
    </div>
  </div>;
}
