...
export default function OfficialTemplateComposer({template,composerProps}:{template:OfficialTemplate;composerProps:ComponentProps<typeof Composer>}){
 ...
 useEffect(()=>{
  ...
  request.current.then(async response=>{
   ...
   if(!response.ok||!data.ref)throw Error(tr('角色素材暫時無法載入，請重試。'));
   if(alive)setRef(data.ref);
  }).catch(()=>{if(alive)setError(tr('角色素材暫時無法載入，請重試。此次沒有生成或扣點。'))});
  return()=>{alive=false};
 },[template.id,router,attempt]);
 if(error)return <div role="alert" className="rounded-xl border border-[#735849] bg-[#211b16] p-5 text-sm"><p>{tr(error)}</p><button type="button" onClick={()=>{request.current=null;setError('');setAttempt(x=>x+1)}} className="mt-3 mr-4 underline">重新載入</button><Link href="/" className="underline">返回首頁</Link></div>;
 if(!ref)return <div role="status" className="rounded-xl bg-[#161c18] p-6 text-sm text-[#a1b4a8]">{tr("正在載入「{title}」與示範角色…", { title: template.title })}</div>;
