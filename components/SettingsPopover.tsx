...
export default function SettingsPopover({
  ...
  const summary =
    mode === "video"
      ? [settings.aspectRatio === "auto" ? tr("自動匹配") : settings.aspectRatio, settings.resolution, `${settings.seconds}s`]
      : mode === "image"
        ? [settings.aspectRatio === "auto" ? tr("自動匹配") : settings.aspectRatio, settings.size, `×${settings.imageCount}`]
        : [tr("最多 {n} tokens", { n: settings.maxTokens })];
        ...
        <div className="p-3">
          <div className="flex items-center justify-between pb-3">
            <span className="text-[13.5px] font-medium">
              {mode === "video" ? tr("影片設定") : mode === "image" ? tr("圖片設定") : tr("生成設定")}
            </span>
            <button
              type="button"
          ...
          <div className="border-t border-[#262626] pt-3">
            {(mode === "video" || mode === "image") && (
              <>
                <div className="pb-2 text-[12.5px] text-[#a8a8a8]">{tr("畫面比例")}</div>
            ...
            {mode === "video" && videoConstraint && (
              <>
                ...
                <div className="pb-2 pt-4 text-[12.5px] text-[#a8a8a8]">
                  {tr("時長")}{videoConstraint.maxSeconds < 30 && <span className="text-[#6d6d6d]">{tr("（此模型最長")} {videoConstraint.maxSeconds} {tr("秒）")}</span>}
                </div>
            ...
            {mode === "image" && imageSizeOptions && (
              <>
                <div className="pb-2 pt-4 text-[12.5px] text-[#a8a8a8]">{tr("尺寸")}</div>
                ...
                <div className="pb-2 pt-4 text-[12.5px] text-[#a8a8a8]">{tr("生成張數")}</div>
            ...
            {mode === "text" && (
              <>
                <div className="pb-2 text-[12.5px] text-[#a8a8a8]">{tr("回覆長度上限（tokens）")}</div>
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                    min={256}
