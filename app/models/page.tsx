...
export default async function ModelsIndex() {
 ...
 return <div className="h-full overflow-y-auto bg-[#0b0c0c] text-[#eeeae1]">
  ...
  <script type="application/ld+json" dangerouslySetInnerHTML={{__html:breadcrumbLd([{name:"首頁",path:"/"},{name:"模型一覽",path:"/models"}])}} />
  <main className="mx-auto max-w-[1360px] px-6 pb-20 pt-12 sm:px-10 lg:px-16 lg:pt-20">
   ...
   {groups.map(g=><section id={g.kind} key={g.kind} className="scroll-mt-8 pt-12 sm:pt-16">
    <div className="mb-8 grid gap-4 md:grid-cols-[1fr_1fr]"><div><p className="mb-3 text-[11px] tracking-[0.2em] text-[#a6b7a9]">{g.number} / {g.label}</p><h2 className="font-serif text-3xl font-normal sm:text-4xl">{g.title}</h2></div><p className="max-w-sm text-sm leading-7 text-[#999e94] md:self-end md:justify-self-end">{g.sub}</p></div>
    <ul className="grid gap-x-10 lg:grid-cols-2">
     {MODEL_PAGES.filter(m=>m.kind===g.kind).map(m=>{const s=modelSpecs(m),p=modelPricing(m,rates);const facts=m.kind==='video'?[s.resolutions.join(' / '),`最長 ${s.maxSeconds} 秒`]:[`${s.sizes.length} 種尺寸`,s.refImages?'可使用參考圖':'文字創作'];return <li key={m.slug} className="border-t border-[#2d312b]">
      <Link href={`/models/${m.slug}`} className="group block h-full py-7 outline-offset-4 transition-colors hover:bg-[#ffffff03] focus-visible:outline focus-visible:outline-[#b6c7bc] sm:py-8">
       ...
       <p className="mt-5 max-w-[38ch] text-[14px] leading-7 text-[#c0c2b7]">{m.tagline}</p>
       <div className="mt-6 flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-t border-[#21251f] pt-4 text-[11px] leading-5 text-[#8d968a]"><span>{facts.join(' · ')}</span>{p&&<span className="whitespace-nowrap">{p.perUnit} 點／{p.unit} 起</span>}</div>
      </Link></li>})}
    </ul>
   </section>)}
   <footer className="mt-12 flex flex-wrap items-center justify-between gap-5 border-t border-[#343630] pt-7 text-xs leading-6 text-[#92998d]"><p>規格與可選功能見各模型內頁。實際點數依生成設定計算。</p><Link className="text-[#c2d2c5]" href="/studio">帶著想法，開始創作 ↗</Link></footer>
  </main>
 </div>;
}
