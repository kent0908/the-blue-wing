  ...
  return (
    <div className="flex h-full min-h-0 flex-col items-center justify-center gap-8 overflow-y-auto px-8 py-12">
      <div className="max-w-lg text-center">
        <h1 className="text-[34px] font-normal text-[#f2f2f2]">{tr("用 OmniHuman 點亮你的數位分身")}</h1>
        <p className="mt-3 text-[14px] text-[#6d6d6d]">
          {tr("上傳一張人像和一段語音，生成會說話、有表情動作的數位分身影片")}
        </p>
        <div className="mt-5 rounded-xl border border-[#3a2e18] bg-[#1a150c] px-4 py-3 text-[12.5px] leading-relaxed text-[#f0c27f]">
          {tr("此頁面為介面預覽 — OmniHuman 1.5 目前尚未串接生成後端，暫時無法實際生成。")}
        </div>
      </div>

      <div className="w-full max-w-[880px] rounded-2xl border border-[#2a2a2a] bg-[#161616]">
        <div className="flex gap-4 px-4 pt-4">
          <UploadTile icon={IconImage} label={tr("人像")} hint={tr("上傳圖片")} />
          <UploadTile icon={IconMic} label={tr("語音")} hint={tr("上傳語音")} />
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            <textarea
              disabled
              rows={3}
              placeholder={tr("（選填）動作與運鏡描述，例如：微笑點頭、鏡頭緩慢推近")}
        ...
        <div className="flex flex-wrap items-center gap-2 p-3">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="text-[11.5px] text-[#5c5c5c]">{tr("尚未開放計費預估")}</span>
            <button
              type="button"
              disabled
