            ...
            <Card>
              ...
              <ul className="mt-2 list-disc space-y-1.5 pl-4 text-[13px] leading-relaxed text-[#c9c9c9]">
                ...
                <li>
                  ...
                  畫面錄成一段 webm 影片，可以直接下載當分鏡參考。停止後可以按「取樣畫面送去影片生成」：這裡要老實說——
                  影片生成用的 API 並沒有「上傳一段影片當運鏡參考」這種功能，所以實際送過去的不是這段影片本身，而是
                  錄製過程中自動抽出的幾張畫面（當多重參考圖，僅 Seedance 系列模型支援，其他模型會被忽略）＋根據鏡頭
                  起訖位置粗略判讀出的運鏡文字提示（例如「鏡頭向右環繞移動」），提示可以在輸入框自行修改。錄下來的影片檔
                  本身不會被 AI 讀取，純粹是給你自己看的分鏡參考。
                </li>
              </ul>
            </Card>
          </section>
