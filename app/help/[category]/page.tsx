  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <article className="mx-auto max-w-[820px] px-6 py-10">
        ...
        <div className="mt-6 space-y-6">
          {c.entries.map((e, i) => (
            <section key={e.question} id={`q${i + 1}`} className="scroll-mt-20 rounded-2xl border border-[#1e1e1e] bg-[#111] p-5">
              <h2 className="text-[15.5px] font-medium text-white">{e.question}</h2>
              <p className="mt-2 whitespace-pre-line text-[14px] leading-7 text-[#c9c9c9]">{e.answer}</p>
            </section>
          ))}
        </div>
        ...
        <footer className="mt-8 text-[12.5px] leading-6 text-[#7a7a7a]">{SITE_DEFINITION}</footer>
      </article>
    </div>
  );
}
