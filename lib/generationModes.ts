...
const durationReason = k("影片自動時長的點數計價尚未開放");
...
export function getGenerationModes(model: string, kind: GenerationKind): GenerationModeOption[] {
  if (kind === "image") {
    const modes: GenerationModeOption[] = [{ id: "universal-reference", label: k("通用參考"), enabled: true }];
    if (/seedream.*5[.-]0.*pro/i.test(model)) modes.push({ id: "layer-separation", label: k("圖層分離"), enabled: LAYER_DECOMPOSITION_AVAILABLE, reason: LAYER_DECOMPOSITION_UNAVAILABLE_REASON });
    return modes;
  }
  ...
  if (modern) return [
    ...
    { id: "video-edit", label: k("影片編輯"), enabled: false, reason: durationReason },
    { id: "video-extend", label: k("影片續寫"), enabled: false, reason: durationReason },
  ];
  if (legacy) return [
    { id: "text-to-video", label: k("文字生影片"), enabled: true },
    { id: "image-to-video", label: k("圖片生影片"), enabled: true },
    ...(/1\.5/.test(model) ? [{ id: "first-last-frame" as const, label: k("首尾幀編輯"), enabled: true }] : []),
  ];
...
export function buildVideoModePayload(input: VideoModeInput): VideoModePayload {
  if (!isGenerationModeSupported(input.model, "video", input.mode)) throw new Error(k("此模型尚未開放所選生成模式"));
  const images = input.imageUrls ?? [];
  const refs = input.references ?? [];
  if (images.some(url => typeof url !== "string" || !url.trim()) || refs.some(ref => !["image", "video", "audio"].includes(ref.type) || typeof ref.url !== "string" || !ref.url.trim())) throw new Error(k("參考素材格式不正確"));
  if (images.length && refs.length) throw new Error(k("首尾幀與多模態參考不可同時使用"));
  ...
  if (input.mode === "text-to-video") {
    ...
    if (images.length) throw new Error(k("此模式請透過參考素材欄位提供圖片"));
    if (input.mode === "subject-reference" && !refs.length) throw new Error(k("主體參考需要至少一份素材"));
    if (refs.length) {
