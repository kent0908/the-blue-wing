...
export const FAQ_CATEGORIES: FaqCategory[] = [
    ...
    entries: [
      {
        question: "圖層編輯是做什麼的？",
        answer:
          "多圖層合成畫布：疊加多張圖片（資產庫或直接上傳／拖進來）與文字圖層，拖曳、8 點縮放（Shift 鎖比例）、旋轉、對齊吸附、不透明度、混合模式（正片疊底／濾色等 16 種）、非破壞裁切與翻轉、亮度／對比／飽和度／色相等色彩校正，排好之後可以匯出（PNG／JPG／WebP、最高 3 倍）、存到資產庫，或直接送去 AI 生成／融合、當影片首幀。",
      },
      {
        question: "圖層編輯有哪些 AI 修圖功能？",
        answer:
          "選一個圖層後右邊有四個：「一鍵去背」在瀏覽器裡跑（第一次下載約 7MB 模型，之後免費不限次數、不花點數）；「局部重繪」可以用筆刷／橡皮擦／矩形／套索／點選物件（智慧選取）圈出範圍，支援羽化、反轉，還可以一次存多個區域各寫一句描述批次送出，每個結果各成一層；「擴圖」把畫面往外延伸，AI 補上邊界外的內容；每次 AI 修改後都有「前後比對」拖曳比對與一鍵「回復」。",
      },
      {
        question: "圖層編輯的專案會存起來嗎？",
        answer:
          "登入後會自動存到帳號（畫面左上會顯示已儲存／未儲存），換裝置也看得到；每次 AI 生成／重繪／擴圖前都會自動留一個版本，也可以手動「存版本」，之後隨時回到任一版本（回去之前會先把目前狀態存成版本，不會弄丟）。",
      },
      {
  ...
  },
  {
    ...
    entries: [
      ...
      {
        question: "檔案大小有限制嗎？參考圖太小或太大會怎樣？",
        answer:
          "單檔上限 64 MB（瀏覽器直接上傳到儲存空間，不經過伺服器）。當參考圖送去生成時，系統會自動調整大小：短邊小於 320px 的會放大（生成服務要求至少 300px），長邊超過 2048px 的會縮小，手機照片的方向也會自動轉正——所以太小或太大的圖都不會再因為尺寸被拒絕。",
      },
    ],
  },
  {
