...
export { LANDING_SLOTS, isLandingSlot } from "./landingSlots";
...
export async function getLandingMediaMap(): Promise<Record<string, PublicLandingMedia>> {
  ...
  for (const r of rows) {
    out[r.slot] = { kind: r.kind, url: `/api/landing-media/${r.slot}?v=${encodeURIComponent(String(r.updated_at))}`, updatedAt: r.updated_at };
  }
  return out;
}
